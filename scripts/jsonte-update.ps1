...
try {
    $latestResponse = (Invoke-RestMethod -Uri https://api.github.com/repos/MCDevKit/jsonte/releases/latest)
} catch {
...
$current = Convert-Semver (jsonte version).split([Environment]::NewLine)[0].split(' ')[2]

# Compare versions
$result = Test-Semver $latest $current
if (!$result) {
    Write-Host "You are running the latest version of jsonte." -ForegroundColor Green
} else {
    # Inform about update and show changelog
    Write-Host "You are not running the latest version of jsonte." -ForegroundColor Yellow
